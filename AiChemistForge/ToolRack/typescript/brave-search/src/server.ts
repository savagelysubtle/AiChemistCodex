...
import {
  ...
  executeWebSearch,
  executeCodeSearch
} from './braveSearchTools.js';

log('info', "Imported from ./braveSearchTools.js");

// Helper function to wrap tool execution with logging
async function wrapToolExecution(
  toolName: string,
  executor: (args: unknown) => Promise<CallToolResult>,
  args: unknown
...
async function main() {
  ...
  try {
    ...
    server.tool(
      ...
      WEB_SEARCH_TOOL.inputSchema,
      // executeWebSearch
      (args: unknown) => wrapToolExecution(WEB_SEARCH_TOOL.name, executeWebSearch, args) // Use wrapper
    );

    log('info', `Registering tool: ${CODE_SEARCH_TOOL.name}`);
    server.tool(
      CODE_SEARCH_TOOL.name,
      CODE_SEARCH_TOOL.inputSchema,
      // executeCodeSearch
      (args: unknown) => wrapToolExecution(CODE_SEARCH_TOOL.name, executeCodeSearch, args) // Use wrapper
    );
